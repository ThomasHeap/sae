...
def parse_args():
    ...
    parser.add_argument('--offline_explainer', action='store_true', 
                        help='Run explainer offline')
    parser.add_argument("--batch_size", type=int, default=5, help="Batch size for scoring")
...
async def process_layer_directory(layer: int, latents_dir: Path, client, feature_cfg: FeatureConfig, 
                                experiment_cfg: ExperimentConfig, batch_size: int):
    ...
    try:
        ...
        def scorer_postprocess(result, score_dir):
            #make dir if not exist
            with open(explanation_dir / f"{score_dir}/{result.record.feature}_score.txt", "wb") as f:
        ...
        #make dirs
        Path(explanation_dir / "detection").mkdir(parents=True, exist_ok=True)
        Path(explanation_dir / "fuzz").mkdir(parents=True, exist_ok=True)
        
        scorer_pipe = Pipe(process_wrapper(
            DetectionScorer(client, tokenizer=dataset.tokenizer, batch_size=batch_size,verbose=False,log_prob=True),
            preprocess=scorer_preprocess,
            postprocess=partial(scorer_postprocess, score_dir="detection"),
        ),
        process_wrapper(
            FuzzingScorer(client, tokenizer=dataset.tokenizer, batch_size=batch_size,verbose=False,log_prob=True),
            preprocess=scorer_preprocess,
...
async def main():
    ...
    if args.api_key and not args.offline_explainer:
        client = OpenRouter("openai/gpt-4o-mini", api_key=args.api_key)
    else:
        #client = Offline("meta-llama/Llama-3.1-8B-Instruct",max_memory=0.8,max_model_len=5120, num_gpus=1)
    ...
    for layer in range(5):
        tasks.append(process_layer_directory(
            layer, latents_dir, client, feature_cfg, experiment_cfg, args.batch_size
        ))
